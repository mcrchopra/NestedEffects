...
def get_missing_nodes_nearest_neighbors(full_similarity_matrix, all_nodes, cosine_matrix, pearson_matrix, xy_file,
    ...
    for index, row in cosine_distance_matrix_missing_nodes.iterrows():
        ...
        row = row.rank(method = "max")
        number_cosine = len(row[row >= 3206])
        cosine_distance_matrix_missing_nodes.ix[index][row < 3206] = np.nan 
        pearson_correlation_matrix_missing_nodes.ix[index][row <3206] = np.nan

        #Rank row of pearson matrix (top 6 of the 100 that are there from cosien)
        pearson_row = pearson_correlation_matrix_missing_nodes.ix[index].rank(method = "max")
        missing_nodes_nearest_neighbors = pearson_correlation_matrix_missing_nodes.ix[index][pearson_row > (number_cosine-6)]
